...
import com.app.service.CustomeUserDetails;
...
public class JwtUtils {
	...
	public String generateJwtToken(Authentication authenticateDetails)
	{
		log.info("generate jwt token"+authenticateDetails);
		CustomeUserDetails userPrincipal = (CustomeUserDetails) authenticateDetails.getPrincipal();
	...
	{
		return Jwts.parser().// Returns a new JwtParser instance used to parse JWT strings.
				setSigningKey(jwtSecret).// Sets the signing key used to verify JWT digital signature.
				parseClaimsJws(token).getBody().getSubject();// Parses the signed JWT returns the resulting Jws<Claims> instance
